...
CREATE TABLE IF NOT EXISTS free_roam.`clients__users__mapping` (
    `client_id`         INT(12)            NOT NULL,
    `user_id`           INT(12)            NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

ALTER TABLE free_roam.`clients__users__mapping`
...
CREATE TABLE IF NOT EXISTS free_roam.`subcription_plans` (
    `id`                                INT(12)         NOT NULL    AUTO_INCREMENT,
    `name`                              VARCHAR(80),
    `validity`                          TIME,
    `query_count`                       INT,
    `status`                            ENUM('ACTIVE', 'INACTIVE'),
    `data_accessiblilty_start_date`     DATE,
    `data_accessiblilty_end_date`       DATE,
    `created_on`                        DATETIME        NOT NULL    DEFAULT NOW(),
    `updated_on`                        DATETIME        NOT NULL    DEFAULT NOW(),
);
...
CREATE TABLE IF NOT EXISTS free_roam.`client_subscriptions` (
    `id`                                INT(12)         NOT NULL    AUTO_INCREMENT,
    `client_id`                         INT(12)         NOT NULL,
    `subcription_plan_id`               INT(12)         NOT NULL,
...
CREATE TABLE free_roam.`graphs` (
    ...
    `name`                              VARCHAR(256),
    `query_id`                          BIGINT(20),
    `x_axis_column`                     VARCHAR(64),
    `y_axis_column`                     VARCHAR(64),
    `z_axis_column`                     VARCHAR(64),
    `status`                            ENUM('ACTIVE', 'INACTIVE'),
    `created_on`                        DATETIME        NOT NULL    DEFAULT NOW(),
    `updated_on`                        DATETIME        NOT NULL    DEFAULT NOW(),
);
...
CREATE TABLE free_roam.`dashboard__graph__mapping` (
    `dashboard_id`                      INT(12)         NOT NULL    AUTO_INCREMENT,
    `graph_id`                          INT(12)         NOT NULL    AUTO_INCREMENT,
    `name`                              VARCHAR(256),
    `row_position`                      INT(2),
